...
- (void)viewDidLoad
{
    ...
    self.sImageView.image = appDelegate.Pass_NameImage;
     ...
     self.sImageView.contentMode = UIViewContentModeScaleAspectFit;
    
    //写真に白い縁をつける  
    CALayer *layer_sImageView = [self.sImageView layer];
    [layer_sImageView setMasksToBounds:YES];
    [layer_sImageView setBorderWidth: 3.f];
    [layer_sImageView setBorderColor:[RGBA(139, 115, 85, 1.0) CGColor]];
...
//    self.book1Image.contentMode = UIViewContentModeCenter;//サイズを自動調整
    ...
    BookData2_SouthAmerica  = [@[
                               ...
                               @"ナスカの壷\nペルーからの手紙",
                               @"エバースマイル・ニュージャージー",
                               @"リヤカーマン\nアンデスを越える―アタカマ砂漠、アンデス山脈越え1000キロ徒歩横断",
                               @"カールじいさんと空飛ぶ家"
                               ]mutableCopy];
    BookData2_Eourope  = [@[
                          @"北アルプス\nテントを背中に山の旅へ ",
...
- (void)myView1_Tapped:(UITapGestureRecognizer *)sender
{
    ...
    self.navigationController.toolbar.tintColor =RGBA(139, 115, 85, 1.0);
